...
class SyncHydrangeaCollection implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $baseUrl = 'https://hydrangea.coffee';
    protected $roastUrls = [];
    ...
    public function handle(): void
    {
        ...
        foreach ($coffees as $index => $coffee) {
            $nameElement = $coffee->querySelector('a.full-unstyled-link b');
            $priceElement = $coffee->querySelector('.price-item--regular');

            $name = $nameElement ? trim( $nameElement->textContent ) : '';
            $price = $priceElement ? $this->formatPrice( $priceElement->textContent ) : '';
            $productUrl = $this->baseUrl.$coffee->querySelector('a.full-unstyled-link')->getAttribute('href');
            ...
            // if( $roast->wasRecentlyCreated ){
                ...
                SyncHydrangeaRoast::dispatch($roast)
    ...
    {
        $collectionUrl = $this->baseUrl.'/collections/active';
