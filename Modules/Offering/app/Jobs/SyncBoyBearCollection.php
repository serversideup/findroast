...
class SyncBoyBearCollection implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $baseUrl = 'https://theboyandthebear.com';
    protected $roastUrls = [];
    ...
    public function handle(): void
    {
        ...
        foreach ($coffees as $index => $coffee) {
            $name = $coffee->querySelector('h5.product_name')->textContent;
            $price = $this->formatPrice($coffee->querySelectorAll('a.product_button .text-size-small')[1]->textContent);
            $productUrl = $this->baseUrl.$coffee->querySelector('a.product_button')->getAttribute('href');
            ...
            if( $roast->wasRecentlyCreated ){
                // We delay the sync out of respect for The Boy and the Bear
                SyncBoyBearRoast::dispatch($roast)
    ...
    {
        $collectionUrl = $this->baseUrl.'/collections/coffee-1';
        ...
        $coffees = $dom->querySelectorAll('.content_collection-list .product_content');
