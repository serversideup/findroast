...
class SyncPtsCollection implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    protected $baseUrl = 'https://ptscoffee.com/';
    protected $roastUrls = [];
    ...
    public function handle(): void
    {
        ...
        foreach ($coffees as $index => $coffee) {
            $name = $coffee->querySelector('.product-grid-item--link .visually-hidden')->textContent;
            $price = $this->formatPrice($coffee->querySelector('p.price_wrapper')->textContent);
            $productUrl = $this->baseUrl.$coffee->querySelector('.product-grid-item--link')->getAttribute('href');
            ...
            if( $roast->wasRecentlyCreated ){
                // We delay the sync out of respect for PT's Coffee
                SyncPtsRoast::dispatch($roast)
    ...
    /**
     * Load the coffee products from the PT's Coffee collection page.
     * 
     * @return \DOMNodeList
     */
    protected function loadCoffeeProducts()
    {
        ...
        $coffees = $dom->querySelectorAll('.product-grid-item');
    ...
    /**
     * Format the price from the PT's Coffee website.
     * 
     * @param string $price
     * @return string
     */
