<template>
    <Drawer :show="open" @close="close" max-width="xl" bg-color="bg-stone-50">
        ...
        <template #subtitle>
            Update brew method information
        </template>
                ...
                <div class="bg-white rounded-lg border border-stone-200 shadow-sm p-6 space-y-5">
                    <div>
                        ...
                        <TextInput
                            ...
                            v-model="form.name"
                            placeholder="e.g., Pour Over, French Press, Aeropress"/>
                        <p class="mt-1.5 text-xs text-stone-500">
                            The display name for this brew method
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex items-center gap-x-4">
                            ...
                            <div
                                class="w-16 h-16 rounded-lg overflow-hidden bg-stone-100 flex items-center justify-center border-2 border-stone-200"
                                v-else-if="brewMethod && brewMethod.icon != '' && brewMethod.icon != null">
                                <img :src="brewMethod.icon" class="w-full h-full object-cover"/>
                            </div>
        ...
        <template #footer>
            <div class="w-full flex items-center justify-between gap-x-4 px-6 py-4 bg-white border-t border-stone-200">
                ...
                <PrimaryButton @click="updateBrewMethod()">Update Brew Method</PrimaryButton>
            </div>
        </template>
    </Drawer>
</template>
...
const promptBusListener = ( event, data ) => {
    if ( event === 'prompt-edit-brew-method' ) {
        open.value = true;
        setBrewMethod(data);
    }
}
...
const updateBrewMethod = () => {
    form.post('/platform/brew-methods/'+brewMethod.value.id, {
        onSuccess: () => {
...
const close = () => {
    ...
    iconFile.value = null;
    brewMethod.value = null;
}

const setBrewMethod = (data) => {
    brewMethod.value = data;
    form.name = data.name;
    form.icon = data.icon;
}
