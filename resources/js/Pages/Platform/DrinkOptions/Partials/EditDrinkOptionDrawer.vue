<template>
    <Drawer :show="open" @close="close" max-width="xl" bg-color="bg-stone-50">
        ...
        <template #subtitle>
            Update drink option information
        </template>
                ...
                <div class="bg-white rounded-lg border border-stone-200 shadow-sm p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex items-center gap-x-4">
                            ...
                            <div
                                class="w-16 h-16 rounded-lg overflow-hidden bg-stone-100 flex items-center justify-center border-2 border-stone-200"
                                v-else-if="drinkOption && drinkOption.icon != '' && drinkOption.icon != null">
                                <img :src="drinkOption.icon" class="w-full h-full object-cover"/>
                            </div>
        ...
        <template #footer>
            <div class="w-full flex items-center justify-between gap-x-4 px-6 py-4 bg-white border-t border-stone-200">
                ...
                <PrimaryButton @click="updateDrinkOption()">Update Drink Option</PrimaryButton>
            </div>
        </template>
    </Drawer>
</template>
...
const updateDrinkOption = () => {
    form.post('/platform/drink-options/'+drinkOption.value.id, {
        onSuccess: () => {
            notificationBus.emit('show', {
                title: 'Drink Option updated',
            });
            close();
        }
    });
...
const close = () => {
    ...
    iconFile.value = null;
    drinkOption.value = null;
}

const setDrinkOption = (data) => {
    drinkOption.value = data;
    form.name = data.name;
    form.icon = data.icon;
}
