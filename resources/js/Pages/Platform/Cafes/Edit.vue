        ...
        <form @submit.prevent="submit" @keydown.enter.prevent>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-lg border border-stone-200 shadow-sm">
                        ...
                        <div class="px-6 py-5 space-y-5">
                            ...
                            <div v-show="!enterAddress && editPlace">
                            ...
                            <!-- Manual Entry Name -->
                            <div v-show="enterAddress || !placeFound">
                            ...
                            <div>
                                ...
                                <div v-if="primaryImageState == 'cafe-primary-image'" class="mt-2 h-40 rounded-lg overflow-hidden border-2 border-stone-200"
                                    :style="{
                                    ...
                                    <div class="text-center">
                                        ...
                                        <div
                                            ...
                                            <label
                                                ...
                                                <span>{{ primaryImageState === 'no-primary-image' ? 'Upload a file' : 'Change image' }}</span>
                                                <input
                    ...
                    <div v-show="enterAddress || !placeFound" class="bg-white rounded-lg border border-stone-200 shadow-sm">
                        <div class="px-6 py-4 border-b border-stone-200">
            ...
            <div class="mt-8 flex items-center justify-between gap-4 p-6 bg-white rounded-lg border border-stone-200 shadow-sm">
                ...
                <PrimaryButton
                    ...
                    {{ form.processing ? 'Updating...' : 'Update Cafe' }}
                </PrimaryButton>
            </div>
        </form>
    </div>
...
const form = useForm({
    _method: 'PUT',
    company_id: cafe.value.company_id,
    name: cafe.value.name,
    status: cafe.value.status,
    primary_image: '',
    description: cafe.value.description,
    country: cafe.value.country,
    address: cafe.value.address,
    city: cafe.value.city,
    state: cafe.value.state,
    province: cafe.value.province,
    territory: cafe.value.territory,
    zip: cafe.value.zip,
    latitude: cafe.value.latitude,
    longitude: cafe.value.longitude,
    yelp_url: cafe.value.yelp_url,
    google_place_id: cafe.value.google_place_id,
    brew_methods: [],
    amenities: [],
    drink_options: [],
});
...
onMounted( async () => {
    form.brew_methods = cafe.value.brew_methods.map( method => method.id );
    form.amenities = cafe.value.amenities.map( amenity => amenity.id );
    form.drink_options = cafe.value.drink_options.map( option => option.id );
    ...
    if( cafe.value.latitude && cafe.value.longitude ){
        setCurrentLocation( cafe.value.latitude, cafe.value.longitude );
    }
});
...
const primaryImageState = computed(() => {
    if( cafe.value.primary_image != null && cafe.value.primary_image != '' && form.primary_image == '' ){
        return 'cafe-primary-image';
...
const editLocation = () => {
    ...
    placeFound.value = false;
    enterAddress.value = true;
}
...
const submit = () => {
    form.post('/platform/cafes/'+cafe.value.id, {
        onSuccess: () => {
